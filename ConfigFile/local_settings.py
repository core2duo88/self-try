...
#
# Use /mnt/data when large files are being uploaded
#
...
# TODO (tjones): setting this false to work around upstream bug
# https://bugs.launchpad.net/horizon/+bug/1597557
...
# The default domain that appears at the login screen when authenticating
# into Hoirzon. The domain field will be prepopulated with this value.
# However, this setting only affects the default if the last used domain
# was unset. Horizon will cache the last used domain name.
OPENSTACK_DEFAULT_LOGIN_DOMAIN = 'Default'
...
HORIZON_CONFIG['help_title'] = "Something went wrong!"

HORIZON_CONFIG['help_message'] = "An unexpected error has occurred. Try refreshing the page. If that does not help, contact your local administrator."

# TODO (tjones) enable the vmware overrides
#HORIZON_CONFIG['customization_module'] = "horizon.vmware.overrides"
...
HORIZON_CONFIG["password_validator"] = {
            ...
            "help_text": _("Your password does not meet the requirements."
                "It must contain at least one upper case character, one lower"
                "case letter, a number, one special character, and be "
...
AVAILABLE_REGIONS = [('http://10.154.9.82:5000/v3','VIO'),]
...
OPENSTACK_NEUTRON_NETWORK = {
    ...
    'enable_quotas': True,
    'enable_ipv6': False,
    'enable_router_type': True,
    'enable_router_size': True,
    ...
    'supported_provider_types': ['flat', 'vlan', 'portgroup', 'vxlan'],
...
OPENSTACK_IMAGE_BACKEND = {
    ...
    'adapter_type' : [  ('', _('Select adapter type')),
                        ('lsiLogic', _('SCSI (Parallel)')),
                        ('lsiLogicsas', _('SCSI (SAS)')),
                        ('ide', _('IDE')),
                        ('paraVirtual', _('ParaVirtual')),
                        ('busLogic', _('Bus Logic'))],
    'disk_type' : [ ('preallocated', _('Preallocated')),
                    ('sparse', _('Sparse')),
                    ('streamOptimized', _('Stream Optimized'))
    ]
}
...
IMAGE_CUSTOM_PROPERTY_TITLES = {
    ...
    "vmware_adaptertype": _("Disk Adapter Type"),
    "vmware_disktype": _("Disk Provisioning Type"),
    "vmware_ostype": _("OS Type"),
    "hw_vif_model": _("Virtual Network Interface"),
}
...
AVAILABLE_THEMES = [
    ...
    ('vmware', 'VMware', 'themes/vmware'),
]

DEFAULT_THEME = 'vmware'
SELECTABLE_THEMES = [
    ('vmware', 'VMware', 'themes/vmware'),
]
...
HORIZON_CONFIG['help_url'] = "https://www.vmware.com/products/openstack"
...
os_types = []
os_types.append(('',''))
with open('/etc/openstack-dashboard/gos.vmsg.txt', 'r') as f:
     for line in f.readlines():
         if line.startswith('guestos.'):
            temp = line.split(' = ')
            (os_type, os_label) = (temp[0].split('.')[1], temp[1].strip('\n'))
            os_types.append((os_type, os_label.strip('"')))

OS_TYPES = { 'os_types': os_types }

SITE_BRANDING = "VMware Integrated OpenStack"
